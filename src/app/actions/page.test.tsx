...
import { DUMMY_REDUCTION_ACTIONS } from "@/lib/reduction-actions";
...
describe("ActionsPage", () => {
  ...
  it("renders the dummy action list with the user's classification badge", () => {
    renderWithDiagnosis(<ActionsPage />, { electricFile, result: fakeResult });

    expect(
      screen.getByRole("heading", {
        name: `맞춤 감축 액션 ${DUMMY_REDUCTION_ACTIONS.length}가지`,
      }),
    ).toBeInTheDocument();
    expect(
    ...
    expect(screen.getByText("-120 kg/년")).toBeInTheDocument();
  ...
  it("shows a running total of expected reduction as actions are selected", async () => {
    ...
    expect(screen.getByText("-205 kg CO₂/년")).toBeInTheDocument();
  });
