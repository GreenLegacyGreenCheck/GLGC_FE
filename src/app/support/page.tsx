    ...
    Promise.all(
      selectedActionCodes.map((code) =>
        getActionPolicy(
          result.diagnosisId,
          code,
          aiInsight?.actions.find((a) => a.code === code)?.reason,
        ),
      ),
    ).then((results) => {
      if (!isCancelled) {
            ...
            <div className="mt-6 space-y-4">
              {programs.map((program, index) => (
                <div
                  ...
                  className="rounded-2xl border border-[#eef3f0] bg-white overflow-hidden"
                >
                  {/* 헤더: 기관명 + 액션 태그 */}
                  <div className="flex items-center justify-between px-5 pt-4">
                    {program.source ? (
                      ...
                      <span className="rounded-full bg-[#eef8f3] px-2.5 py-0.5 text-xs font-black text-[#1ba77d]">
                        {program.actionTitle}
                      </span>
                    )}
                  </div>
                  ...
                  <div className="mt-3 flex flex-wrap gap-2 px-5">
                    ...
                    {program.actionTitle && program.source ? (
                      <span className="rounded-full bg-[#eef8f3] px-3 py-1 text-xs font-bold text-[#1ba77d]">
                        {program.actionTitle}
                      </span>
                    ) : null}
                    {program.difficulty ? (
                      <span className="rounded-full bg-[#fbe7c8] px-3 py-1 text-xs font-bold text-[#9a5b1f]">
                        {program.difficulty}
                      </span>
                    ) : null}
                  </div>
                  ...
                  {/* 필요 서류 */}
                  {program.documents ? (
                    <p className="mt-3 px-5 text-xs font-bold text-[#789b8c]">
